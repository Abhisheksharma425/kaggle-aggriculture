...
def agent(obs):
    ...
    for x, y in unlocked:
        ...
        if t is None:
            ...
            if kind == "WEED":
                ...
                # Check if it needs fertilizer (only fertilize premium crops to save fertilizer)
                fert_until = t.get("fertilized_until_day", -1)
                if fert_until < day and crop in ("STRAWBERRY", "MELON"):
    ...
    for item, qty in shed.items():
        if qty > 0 and item not in ("WHEAT", "COW", "SHEEP", "FERTILIZER"):
            market.append(["SELL", item, qty])
            
    # Sell excess fertilizer if we have a massive stockpile
    fert_in_shed = shed.get("FERTILIZER", 0)
    if fert_in_shed > 25:
        market.append(["SELL", "FERTILIZER", fert_in_shed - 20])
    ...
    # --- PHASE 1 & 2: Aggressive Livestock purchasing ---
    if day <= 22:
        if total_sheep < 4 and money >= 500 and animals_in_shed == 0:
            market.append(["BUY_ANIMAL", "SHEEP", 1])
            money -= 500
        
        target_cows = 8 if day >= 6 else 1
        if total_cows < target_cows and money >= 400 and animals_in_shed == 0:
            market.append(["BUY_ANIMAL", "COW", 1])
            money -= 400

    # --- Seed Purchasing Logic (Phase 2 vs Phase 3) ---
    ...
    available_slots = max(0, len(empty_tiles) - needed_pastures)
    
    if day < 19:
        # PHASE 1 & 2: Premium Crops (Strawberries & Melons) + Enough Wheat to Feed Animals
        target_wheat_total = min(available_slots, (total_cows + total_sheep) * 2 + 2)
        premium_slots = max(0, available_slots - target_wheat_total)
        
        target_straw_total = int(premium_slots * 0.7)
        target_melon_total = int(premium_slots * 0.3)
        
        straw_deficit = max(0, target_straw_total - straw_seeds)
        ...
        melon_deficit = max(0, target_melon_total - melon_seeds)
        while melon_deficit > 0 and money >= 200 and len(market) < 8:
            buy_m = min(melon_deficit, int(money // 200), 10)
            if buy_m <= 0: break
        ...
        # Backfill with wheat
        wheat_deficit = max(0, available_slots - straw_seeds - melon_seeds - wheat_seeds)
        while wheat_deficit > 0 and money >= 10 and len(market) < 8:
            buy_w = min(wheat_deficit, int(money // 10), 10)
            if buy_w <= 0: break
        ...
        # PHASE 3: Wheat Sprint (Only fast growing crops)
        wheat_deficit = max(0, available_slots - wheat_seeds - straw_seeds - melon_seeds)
        while wheat_deficit > 0 and money >= 10 and len(market) < 8:
            buy_w = min(wheat_deficit, int(money // 10), 10)
            if buy_w <= 0: break
            market.append(["BUY_SEED", "WHEAT", buy_w])
    ...
    if day <= 28:
        max_hire_cost = 21 if day >= 8 else 13
    ...
    r_fert_sources = list(fertilizer_sources)
    r_fert_targets = list(fertilize_targets)
    ...
    for u_idx, u_pos in enumerate(units):
        ...
        fert_in_inv = inv.get("FERTILIZER", 0)
        has_cow = inv.get("COW", 0) > 0
        has_sheep = inv.get("SHEEP", 0) > 0
        carrying_animal = has_cow or has_sheep
        has_produce = sum(v for k, v in inv.items() if k not in ("WHEAT", "FERTILIZER")) > 0
        ...
        if action is None and here in r_fert_targets and fert_in_inv > 0:
            action = ["FERTILIZE"]
            r_fert_targets.remove(here)

        if action is None and here in r_fert_sources:
            action = ["COLLECT_FERTILIZER"]
            r_fert_sources.remove(here)
        ...
        if action is None and on_shed:
            if has_produce and not carrying_animal:
                action = ["DROP"]
            elif len(r_fert_targets) > 0 and fert_in_inv == 0 and fert_in_shed > workers_fetching_fert and not carrying_animal:
                action = ["PICKUP", "FERTILIZER", 1]
                workers_fetching_fert += 1
                fert_in_shed -= 1
        ...
        if action is None:
            ...
            if carrying_animal:
                for t in r_empasture:
                    if t not in assigned: candidates.append((0, t, ""))
                    
            if wheat_in_inv > 0 and not carrying_animal:
                for t in r_unfed:
                    if t not in assigned: candidates.append((0, t, ""))
                    
            if fert_in_inv > 0 and not carrying_animal:
                for t in r_fert_targets:
                    if t not in assigned: candidates.append((0, t, ""))
            ...
            if fert_in_inv == 0 and len(r_fert_targets) > workers_fetching_fert and not carrying_animal:
                if fert_in_shed > workers_fetching_fert:
                    for st in SHED_TILES:
                        if st not in assigned:
                            candidates.append((0, st, "FETCH_FERT"))
                            break
            ...
            for t in r_harvest:
                if t not in assigned: candidates.append((1, t, ""))
            for t in r_water:
                if t not in assigned: candidates.append((2, t, ""))
            for t in r_care:
                if t not in assigned: candidates.append((3, t, ""))
            for t in r_fert_sources:
                if t not in assigned: candidates.append((3, t, ""))
            for t in r_weeds:
                if t not in assigned: candidates.append((4, t, ""))
            if (wheat_seeds + straw_seeds + melon_seeds) > 0 or need_pastures > 0:
                for t in r_empty:
                    if t not in assigned: candidates.append((5, t, ""))
            if has_produce:
                for st in SHED_TILES:
                    if st not in assigned:
